...
#    Field('parent', 'reference pagetree'),
db.define_table('pagetree',
    Field('page_language',writable=False,readable=False),
    Field('parent', type='integer', writable=False,readable=False,  
          label=T('Parent pagetree')),                
    ...
    Field('url',default='',label=T('Unique name of the URL'),
          requires=(IS_SLUG(),IS_NOT_IN_DB(db,'pagetree.url'))),
    ...
    Field('isMenuitem','boolean',default=True,label=T("When enabled the pagetree will appear in menu or submenu")),
    Field('showChildren','boolean',default=True,label=T("When enabled the subdocuments of this pagetree will be visible")),
    Field('childrenTitle','string',label=T("Name the sub pagetree")),
    Field('image', type='upload', label=T("pagetree icon")),
...
db.define_table('pagetree_archive',                                    
    Field('current_record',db.pagetree),                               
    db.pagetree,
    format = '%(slug) %(modified_on)s', migrate=True)

db.define_table('page_item',
   Field('pagetree', db.pagetree, writable=False,readable=False,
          label=T('page')),  
   Field('tablename', type='string', requires=IS_IN_SET(('page_text','page_image','page_file','page_link','page_youtube','page_facebook','page_twitter','page_form','page_image_slider','page_form','page_faq','page_dealer','page_fbcomments')), writable=False,readable=False,
...
db.page_item.pagetree.requires = IS_IN_DB(db, db.pagetree.id, 'Language: %(language)s %(title)s')

db.define_table('page_text',
   Field('title', type='string',
          label=T('Define if you use subtitles')),
   Field('text_type', type='string', writable=False,readable=False,  
          requires=IS_IN_SET(('html','markmin','textarea')), label=T('Parent pagetree')),                 
...
db.define_table('page_file',
   ...
   Field('file_upload', type='upload',requires=(IS_NOT_EMPTY())))
...
db.define_table('page_twitter',
   Field('twitter', type='string',
         label=T("Twitter User"), requires=(IS_NOT_EMPTY())))
...
db.define_table('page_address',
    Field('name', type='string', label=T('name')),
    Field('address', type='string', label=T('address'), requires=(IS_NOT_EMPTY())),
...
db.define_table('page_slider',
   ...
   Field('image1', type='upload', uploadfolder=os.path.join(request.folder,'static/images/slider'), requires=(IS_NOT_EMPTY()), comment=T("Only first picture is mandatory")),
   ...
   Field('image2', type='upload', uploadfolder=os.path.join(request.folder,'static/images/slider')),
   ...
   Field('image3', type='upload', uploadfolder=os.path.join(request.folder,'static/images/slider')),
   ...
   Field('image4', type='upload', uploadfolder=os.path.join(request.folder,'static/images/slider')),
   ...
   Field('image5', type='upload', uploadfolder=os.path.join(request.folder,'static/images/slider')),
   ...
   Field('image6', type='upload', uploadfolder=os.path.join(request.folder,'static/images/slider')),
   ...
   Field('image7', type='upload', uploadfolder=os.path.join(request.folder,'static/images/slider')),
   ...
   Field('image8', type='upload', uploadfolder=os.path.join(request.folder,'static/images/slider')),
   ...
   Field('image9', type='upload', uploadfolder=os.path.join(request.folder,'static/images/slider')),
   ...
   Field('image10', type='upload', uploadfolder=os.path.join(request.folder,'static/images/slider')),
...
db.define_table('page_fbcomments',
   Field('mod_type', type='string', requires=IS_IN_SET(('none','admins','app_id')), default='none', label=T("Moderation type")),
   Field('mod_id', type='string', label=T('Moderation ID')),
   Field('href', type='string', label=T('URL to comment on'), requires=(IS_NOT_EMPTY())),
   Field('width', type='string', label=T('Plugin width in pixels (minimum recommended width: 400)'), requires=(IS_NOT_EMPTY())),
   Field('colorscheme', type='string', requires=IS_IN_SET(('light','dark')), default='light', label=T("Color scheme")),
   Field('num_posts', type='integer', default=10, label=T('Number of posts'), requires=(IS_NOT_EMPTY())))

#Add first 'en' toplevel pagetree if none exist
if (db((db.pagetree.parent==0) & (db.pagetree.page_language=='en-us') & (db.pagetree.isMenuitem==True)).count() == 0):
    db.pagetree.insert(page_language='en-us',
                   parent=0,
                   title='Information',
                   short_title='Info',
                   url='information')

#Add mandatory admin group. Please give your first user admin right via admin
if (db(db.auth_group.role == 'admins').count() == 0):
    auth.add_group('admins', 'admin group')

# TODO: write code that makes first user admin
# Or figure out some other smart way to enable yourself without a default pass.